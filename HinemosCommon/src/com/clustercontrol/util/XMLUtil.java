...
/**
 * JAX-WSで使用するXML 1.0の規約に従い、使用不可の文字を削除するなど行うUtilityクラス
 * 
 * 参考：
...
public class XMLUtil {
	...
	private static Log m_log = LogFactory.getLog(XMLUtil.class);

	// class variables
	private static final byte[] CHARS = new byte[1 << 16];
	private static final int MASK_VALID = 0x01;
	private static final String PREFIX_HEX = "\\x";
	...
	public static void setReplaceChar(String replaceChar) {
		m_log.info("setReplaceChar() XML Invalid Chars replace to [" + replaceChar + "]");
		XMLUtil.replaceChar = replaceChar;
	}
	...
	public static void setReplace(boolean replace) {
		m_log.info("setReplace() XML Invalid Chars replace is " + replace);
		XMLUtil.replace = replace;
	}

	/**
	 * 入力文字列datavalueについてXMLとして扱えない文字を削除した文字列を返却する
	 * 
	 * @param datavalue
	 * @return
	 */
	public static String ignoreInvalidString(String datavalue) {
		if(m_log.isDebugEnabled()){
			m_log.debug("ignoreInvalidString() input str = " + datavalue);
		}

		// null and empty check
		if(datavalue == null || datavalue.length() == 0){
			if(m_log.isDebugEnabled()){
				m_log.debug("ignoreInvalidString() input str is null or empty string.");
			}
			return datavalue;
		}
		...
		if(existInvalidCharactor(dataarray)){
			return replaceInvalidCharactor(dataarray);
		}
		else{
			return datavalue;
		}
	...
	/**
	 * dataarrayの中にXMLの不正な文字列が含まれるか否か
	 * 
	 * @param dataarray
	 * @return
	 */
	private static boolean existInvalidCharactor(char[] dataarray) {
		if(m_log.isDebugEnabled()){
			m_log.debug("existInvalidCharactor() input str = " + new String(dataarray));
		}
		...
		while (i < datalength) {
			...
			char ch0 = dataarray[i++];
			if (XMLUtil.isInvalid(ch0)) {

				// check if this is a supplemental character
				if (XMLUtil.isHighSurrogate(ch0) && i < datalength) {

					char ch1 = dataarray[i++];
					if (XMLUtil.isLowSurrogate(ch1)
							&& XMLUtil.isSupplemental(XMLUtil.supplemental(ch0,
									ch1))) {
						continue;
					}
				}
	...
	/**
	 * dataarrayの中のXMLの不正な文字を置換する
	 * 
	 * @param dataarray
	 * @return
	 */
	private static String replaceInvalidCharactor(char[] dataarray) {
		m_log.warn("replaceInvalidCharactor() input str = " + new String(dataarray));
			...
			if (XMLUtil.isInvalid(ch0)) {
				...
				if (XMLUtil.isHighSurrogate(ch0) && i < datalength) {
					...
					if (XMLUtil.isLowSurrogate(ch1)
							&& XMLUtil.isSupplemental(XMLUtil.supplemental(ch0,
									ch1))) {
						outBuffer.append(ch0);
						outBuffer.append(ch1);
						continue;
					}
					else{
						//サロゲートペアの文字置換
						if(replace){
							m_log.warn("replaceInvalidCharactor() replace(surrogate pair) at index = " + (i-2) + ", char = " + new String(dataarray, i-2, 2));
							outBuffer.append(replaceChar);
						}
						else{
							outBuffer.append(replaceHexToString(ch0));
							outBuffer.append(replaceHexToString(ch1));
						}
						continue;
					}
				}
				...
				//1byte文字置換
				if(replace){
					m_log.warn("replaceInvalidCharactor() replace at index = " + (i-1) + ", char = " + new String(dataarray, i-1, 1));
					outBuffer.append(replaceChar);
				}
				else{
					outBuffer.append(replaceHexToString(ch0));
		...
		if(m_log.isDebugEnabled()){
			m_log.debug("replaceInvalidCharactor() outoput str = " + outBuffer.toString());
		}
		return outBuffer.toString();
	}

	private static String replaceHexToString(char c){
		if(m_log.isDebugEnabled()){
			m_log.debug("replaceHexToString() input char = " + c);
		}
		// 文字に対応するUnicodeコードを取得します。
		int code = c;

		if(m_log.isDebugEnabled()){
			m_log.debug("replaceHexToString() input char code = " + code);
			m_log.debug("replaceHexToString() return string = " + PREFIX_HEX + Integer.toHexString(code));
		}

		return PREFIX_HEX + Integer.toHexString(code);
	}
	...
	public static void main(String[] args) {
		System.out.println("---- replace = true, replaceChar = ?");
		test(true, "+");
	 ...
	 * @param replaceChar
	 */
	public static void test(boolean replace, String replaceChar){

		// 基本設定
		XMLUtil.setReplace(replace);
		XMLUtil.setReplaceChar(replaceChar);

		// 制御コード
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1 = [" + ((char)0x1) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x2 = [" + ((char)0x2) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x3 = [" + ((char)0x3) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x4 = [" + ((char)0x4) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x5 = [" + ((char)0x5) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x6 = [" + ((char)0x6) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x7 = [" + ((char)0x7) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x8 = [" + ((char)0x8) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("OK : 0x9 = [" + ((char)0x9) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("OK : 0xa = [" + ((char)0xa) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0xb = [" + ((char)0xb) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0xc = [" + ((char)0xc) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("OK : 0xd = [" + ((char)0xd) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0xe = [" + ((char)0xe) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0xf = [" + ((char)0xf) + "]"));
		System.out.println();
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x10 = [" + ((char)0x10) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x11 = [" + ((char)0x11) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x12 = [" + ((char)0x12) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x13 = [" + ((char)0x13) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x14 = [" + ((char)0x14) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x15 = [" + ((char)0x15) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x16 = [" + ((char)0x16) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x17 = [" + ((char)0x17) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x18 = [" + ((char)0x18) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x19 = [" + ((char)0x19) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1a = [" + ((char)0x1a) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1b = [" + ((char)0x1b) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1c = [" + ((char)0x1c) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1d = [" + ((char)0x1d) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1e = [" + ((char)0x1e) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG : 0x1f = [" + ((char)0x1f) + "]"));
		System.out.println();
		System.out.println(XMLUtil.ignoreInvalidString("OK : 0x20 = [" + ((char)0x20) + "]"));

		// サロゲートペアを持つ文字
		System.out.println(XMLUtil.ignoreInvalidString("OK : U+2000B = [" + ((char)0xd840) + ((char)0xdc0B) + "]"));
		System.out.println(XMLUtil.ignoreInvalidString("NG :         = [" + ((char)0xd840) + ((char)0x0) + "]"));
