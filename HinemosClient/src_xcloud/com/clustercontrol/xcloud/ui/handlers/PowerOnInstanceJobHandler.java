...
public class PowerOnInstanceJobHandler extends AbstaractCloudOptionJobHandler {
	...
	protected String getJobId() {
		...
		if (jobid.length() > CloudUtil.jobIdMaxLength) {
			...
			jobid = String.format("%s_%s_1-poweron", instance.getLocation().getId(), instance.getId().substring(diff, instance.getId().length()));
		}
	...
	protected String cutJobId(int num) {
		String jobid = String.format("%s_%s_i-poweron", instance.getLocation().getId(), instance.getId());
		int diff = jobid.length() - CloudUtil.jobIdMaxLength;
		jobid = String.format("%s_%s_i-poweron", instance.getLocation().getId(), instance.getId().substring(diff + num, instance.getId().length()));
		return jobid;
	}
	...
	protected ResourceActionEnum getAction() {
		return ResourceActionEnum.POWERON;
	}
}
