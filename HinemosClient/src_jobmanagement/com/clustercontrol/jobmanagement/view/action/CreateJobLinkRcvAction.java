...
/**
 * ジョブ[実行契機]ビューの「ジョブ連携受信実行契機」のクライアント側アクションクラス<BR>
 *
 */
public class CreateJobLinkRcvAction extends AbstractHandler {
	/** ログ */
	private static Log m_log = LogFactory.getLog(CreateJobLinkRcvAction.class);
	/** アクションID */
	public static final String ID = CreateJobLinkRcvAction.class.getName();
	...
	/**
	 * ジョブ[実行契機]ビューの「ジョブ連携受信実行契機」が押された場合に、実行契機[ジョブ連携受信実行契機]を作成します。
	 * <p>
	 * <ol>
	 * <li>ジョブ[ジョブ連携受信実行契機の作成・変更]ダイアログを表示します。</li>
	 * <li>ジョブ[実行契機]ビューを更新します。</li>
	 * </ol>
	 *
	 * @see org.eclipse.core.commands.IHandler#execute
	 * @see com.clustercontrol.jobmanagement.dialog.FileKickDialog
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (viewPart instanceof JobKickListView) {
			...
			JobKickDialog dialog = new JobKickDialog(HandlerUtil.getActiveWorkbenchWindow( event ).getShell(),
					managerName, null, JobKickResponse.TypeEnum.JOBLINKRCV, PropertyDefineConstant.MODE_ADD);
			dialog.open();
			composite.update();
		}
		return null;
