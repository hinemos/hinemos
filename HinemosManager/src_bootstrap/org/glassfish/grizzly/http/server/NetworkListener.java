...
public class NetworkListener {
	...
	private static final Log HINEMOS_LOG = LogFactory.getLog(NetworkListener.class);

	/**
	 * The default network host to which the {@link HttpServer} will bind to in
	 * order to service <code>HTTP</code> requests.
	 */
	public static final String DEFAULT_NETWORK_HOST = "0.0.0.0";

	/**
	 * The default network port to which the {@link HttpServer} will bind to in
	 * order to service <code>HTTP</code> requests.
	 */
	public static final int DEFAULT_NETWORK_PORT = 8080;

	/**
	 * The network host to which the
	 * <code>HttpServer<code> will bind to in order to service <code>HTTP</code>
	 * requests. If not explicitly set, the value of
	 * {@link #DEFAULT_NETWORK_HOST} will be used.
	 */
	private String host = DEFAULT_NETWORK_HOST;

	/**
	 * The network port to which the
	 * <code>HttpServer<code> will bind to in order to service <code>HTTP</code>
	 * requests. If not explicitly set, the value of
	 * {@link #DEFAULT_NETWORK_PORT} will be used.
	 */
	private int port = DEFAULT_NETWORK_PORT;
	/**
	 * The flag indicates if the
	 * <code>HttpServer<code> will be bounnd to an inherited Channel.
	 ...
	 * in order to service <code>HTTP</code> requests. If not explicitly set,
	 * the value of {@link #port} will be used.
	 */
	...
	/**
	 * TCP Server {@link Connection} responsible for accepting client
	 * connections
	 */
...

	{
		...
		transport = builder.setIOStrategy(SameThreadIOStrategy.getInstance())
				.setWorkerThreadPoolConfig(
						ThreadPoolConfig.defaultConfig().setPoolName("Grizzly-worker").setCorePoolSize(coresCount)
								.setMaxPoolSize(coresCount).setMemoryManager(builder.getMemoryManager()))
				.build();
	}
	...
	/**
	 * Flag indicating whether or not the chunked transfer encoding is enabled.
	 * Defaults to <code>true</code>.
	 */
	private boolean chunkingEnabled = true;
	/**
	 * Configuration for the {@link SSLEngine} that will be used for secure
	 * listeners.
	 */
	 ...
	 * <p>
	 * Constructs a new <code>NetworkListener</code> using the specified
	 * <code>name</code>, which, depending on <code>isBindToInherited</code>
	 * will or will not be bound to an inherited Channel.
	 * </p>
	 *
	 * @param name
	 *            the logical name of the listener.
	 * @param isBindToInherited
	 *            if <tt>true</tt> the <code>NetworkListener</code> will be
	 *            bound to an inherited Channel, otherwise default
	 *            {@link #DEFAULT_NETWORK_HOST} and
	 *            {@link #DEFAULT_NETWORK_PORT} will be used.
	 * 
	 ...
	 * <p>
	 * Constructs a new <code>NetworkListener</code> using the specified
	 * <code>name</code> and <code>host</code>. The listener's port will default
	 * to {@link #DEFAULT_NETWORK_PORT}.
	 * </p>
	 *
	 * @param name
	 *            the logical name of the listener.
	 ...
	 *            the network host to which this listener will bind.
	 * @param portRange
	 *            the network port range to which this listener will bind..
	 */
	 ...
	 *         If the {@link HttpServer} has not been started yet - the returned
	 *         value may be: <tt>-1</tt>, if {@link PortRange} will be used to
	 *         bind the listener; <tt>0</tt>, if the port will be assigned by
	 *         OS; <tt>0 < N < 65536</tt>, the port this listener will be bound
	 *         to. If {@link HttpServer} has been started - the value returned
	 *         is the port the this listener is bound to.
	 */
	public int getPort() {
		return port;
	...
	/**
	 * @return the network port range to which this listener is configured to
	 *         bind to.
	 */
	 ...
	 * <p/>
	 * <p>
	 * Attempts to change the transport implementation while the listener is
	 * running will be ignored.
	 * </p>
	 *
	 * @param transport
	 *            a custom {@link TCPNIOTransport} implementation.
	 */
	...
	/**
	 * Return the array of the registered {@link AddOn}s. Please note, possible
	 * array modifications wont affect the {@link NetworkListener}'s addons
	 * list.
	 *
	 ...
	 * 
	 * @param addon
	 *            the {@link AddOn} to be registered.
	 *
	 * @return <tt>true</tt>, if the {@link AddOn} wasn't registered before,
	 *         otherwise the existing {@link AddOn} will be replaced and this
	...
	/**
	 * @return <code>true</code> if the HTTP response bodies should be chunked
	 *         if not content length has been explicitly specified.
	 */
	...
	/**
	 * Enable/disable chunking of an HTTP response body if no content length has
	 * been explictly specified. Chunking is enabled by default.
	 *
	 * @param chunkingEnabled
	 *            <code>true</code> to enable chunking; <code>false</code> to
	 *            disable.
	 */
	 ...
	 * <p/>
	 * <p>
	 * Attempts to change this value while the listener is running will be
	 * ignored.
	 * </p>
	 *
	 * @param secure
	 *            if <code>true</code> this listener will be secure.
	 */
	 ...
	 *         HttpServer is running behind HTTP gateway like reverse proxy or
	 *         load balancer.
	 *
	 * @since 2.3.18
	 */
	 ...
	 *
	 * If the specified value is less than zero, then there may be an unlimited
	 * number of headers (memory permitting).
	 *
	 * @since 2.2.11
	 */
	...
	/**
	 * <p>
	 * Specifies the {@link FilterChain} to be used by the
	 * {@link TCPNIOTransport} associated with this listener.
	 * </p>
	 * <p/>
	 * <p>
	 * Attempts to change this value while the listener is running will be
	...
	/**
	 * @return the maximum size, in bytes, of all data waiting to be written to
	 *         the associated {@link Connection}. If not explicitly set, the
	 *         value will be -1 which effectively disables resource enforcement.
	 */
	...
	/**
	 * The maximum size, in bytes, of all data waiting to be written to the
	 * associated {@link Connection}. If the value is zero or less, then no
	 * resource enforcement will take place.
	 *
	 ...
	 *
	 * @throws IOException
	 *             if an error occurs when attempting to start the listener.
	 */
		...
		synchronized (lockObj) {
			...
			transport.addShutdownListener(new GracefulShutdownListener() {
				...
				public void shutdownRequested(final ShutdownContext shutdownContext) {
					...
					filterChain.fireEventDownstream(serverConnection, shutdownEvent,
							new EmptyCompletionHandler<FilterChainContext>() {
								...
								public void completed(final FilterChainContext result) {
									...
									if (!tasks.isEmpty()) {
										final ExecutorService shutdownService = Executors
												.newFixedThreadPool(Math.min(5, tasks.size()) + 1);
											...
											public void run() {
												try {
													...
													for (Future<Filter> future : futures) {
														try {
															...
															if (LOGGER.isLoggable(Level.SEVERE)) {
																HINEMOS_LOG.warn(
																		"Error processing shutdown task filter.", e);
															}
														}
													}
													...
													if (LOGGER.isLoggable(Level.WARNING)) {
														HINEMOS_LOG.warn("NetworkListener shutdown interrupted.");
													}
													if (LOGGER.isLoggable(Level.FINE)) {
														HINEMOS_LOG.info(e.toString(), e);
													}
												}
											}
										});
			...
			if (LOGGER.isLoggable(Level.INFO)) {
				HINEMOS_LOG.info(String.format("Started listener bound to [%s]", host + ':' + port));
			}
		}

	}

	public GrizzlyFuture<NetworkListener> shutdown(final long gracePeriod, final TimeUnit timeUnit) {
		synchronized (lockObj) {
	 ...
	 * </p>
	 * Any exceptions thrown during the shutdown process will be propagated to
	 * the returned {@link GrizzlyFuture}.
	 * 
	 ...
	 *
	 * @throws IOException
	 *             if an error occurs when attempting to shut down the listener
	 */
	public void shutdownNow() throws IOException {
		synchronized (lockObj) {
			...
			try {
				...
				if (LOGGER.isLoggable(Level.INFO)) {
					HINEMOS_LOG.info(String.format("Stopped listener bound to [%s]", host + ':' + port));
				}
			} finally {
				state = State.STOPPED;
				if (shutdownFuture != null) {
	...
	public void pause() {
		synchronized (lockObj) {
			...
			state = State.PAUSED;
			if (LOGGER.isLoggable(Level.INFO)) {
				HINEMOS_LOG.info(String.format("Paused listener bound to [%s]", host + ':' + port));
			}
		}
	}
	...
	public void resume() {
		synchronized (lockObj) {
			...
			state = State.RUNNING;
			if (LOGGER.isLoggable(Level.INFO)) {
				HINEMOS_LOG.info(String.format("Resumed listener bound to [%s]", host + ':' + port));
			}
		}
	}
	...
	public String toString() {
		return "NetworkListener{" + "name='" + name + '\'' + ", host='" + host + '\'' + ", port=" + port + ", secure="
				+ secure + ", state=" + state + '}';
	}

	public Object createManagementObject() {
		return MonitoringUtils.loadJmxObject("org.glassfish.grizzly.http.server.jmx.NetworkListener", this,
	...
	private static void validateArg(final String name, final String value) {
		if (value == null || value.length() == 0) {
			throw new IllegalArgumentException(
					"Argument " + name + " cannot be " + (value == null ? "null" : "have a zero length")); // I18n
		}
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().getCompressionMode().name()</tt>
	 */
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().setCompressionMinSize(int)</tt>
	 */
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().getCompressibleMimeTypes()</tt>
	 */
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().setCompressibleMimeTypes(Set&lt;String&gt;)</tt>
	 */
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().getNoCompressionUserAgents()</tt>
	 */
	...
	/**
	 * @deprecated use
	 *             <tt>getCompressionConfig().setNoCompressionUserAgents(Set&lt;String&gt;)</tt>
	 */
	...
	/**
	 * @return The timeout, in seconds, within which a request must complete its
	 *         processing. If not explicitly set, no transaction timeout will be
	 *         enforced.
	 */
	 ...
	 * processing. A value less than or equal to zero will disable this timeout.
	 * Note that this configuration option is only considered when the
