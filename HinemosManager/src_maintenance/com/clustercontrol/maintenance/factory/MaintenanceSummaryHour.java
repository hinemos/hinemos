...
public class MaintenanceSummaryHour extends MaintenanceObject {
	...
	protected int _delete(Long boundary, boolean status, String ownerRoleId, String maintenanceId){
		...
		long timeout = HinemosPropertyCommon.maintenance_collect_summary_hour_history_deletion_timeout.getNumericValue();
		...
		try{
			...
			if(RoleIdConstant.isAdministratorRole(ownerRoleId)){
				synchronized (_deleteLock) {
					List<Date> targetDateList = QueryUtil.selectTargetDateSummaryHourByDateTime(boundary);
				...
				for(int i = 0; i < monitorIdList.size(); i++){
					synchronized (_deleteLock) {
						List<Date> targetDateList = QueryUtil.selectTargetDateSummaryHourByDateTimeAndMonitorId(boundary, monitorIdList.get(i));
	...
	protected int delete(Date targetDate, String monitorId) {
		...
		int ret = QueryUtil.deleteSummaryHourByDateTimeAndMonitorId(
	...
	protected int delete(Date targetDate) {
		//SQL文の実行
		//for HA (縮退判定時間を延ばすため)、シングルには影響なし(0)：タイムアウト値設定
		int ret = QueryUtil.deleteSummaryHourByDateTime(
