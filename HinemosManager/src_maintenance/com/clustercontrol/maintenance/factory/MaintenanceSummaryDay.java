...
public class MaintenanceSummaryDay extends MaintenanceObject {
	...
	protected int _delete(Long boundary, boolean status, String ownerRoleId, String maintenanceId){
		...
		long timeout = HinemosPropertyCommon.maintenance_collect_summary_day_history_deletion_timeout.getNumericValue();
		...
		try{
			...
			if(RoleIdConstant.isAdministratorRole(ownerRoleId)){
				synchronized (_deleteLock) {
					List<Date> targetDateList = QueryUtil.selectTargetDateSummaryDayByDateTime(boundary);
				...
				for(int i = 0; i < monitorIdList.size(); i++){
					synchronized (_deleteLock) {
						List<Date> targetDateList = QueryUtil.selectTargetDateSummaryDayByDateTimeAndMonitorId(boundary, monitorIdList.get(i));
