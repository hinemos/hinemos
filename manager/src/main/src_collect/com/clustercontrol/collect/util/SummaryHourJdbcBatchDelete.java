...
/**
 *  SummaryHourにマッピングするテーブルからデータを削除するクラス
 */
public class SummaryHourJdbcBatchDelete extends JdbcBatchQuery {
	private static final String SQL = "DELETE FROM log.cc_collect_summary_hour "
			+ "WHERE collector_id = ? "
			+ "AND time >= ? "
			+ "AND time <= ? ";
	...
	public SummaryHourJdbcBatchDelete(List<CollectKeyInfo> entities, Long fromTime, Long toTime) {
