 ...
 * @since 6.1.0
 */
public class BinaryPatternInfoComposite extends Composite {
	...
	/** 判定情報一覧 コンポジット。 */
	private BinaryListComposite m_infoList = null;
	...
	/** メッセージにデフォルトを入れるフラグ(#[Binary_LINE]) */
	 ...
	 */
	public BinaryPatternInfoComposite(Composite parent, int style, ArrayList<TableColumnInfo> tableDefine,
			boolean lineFlag) {
		super(parent, style);
	...
	private void initialize(ArrayList<TableColumnInfo> tableDefine) {
		 ...
		 * 文字列監視判定情報一覧
		 */
		this.m_infoList = new BinaryListComposite(this, SWT.BORDER, tableDefine);
		WidgetTestUtil.setTestId(this, "binarypatternlist", m_infoList);
		...
		this.m_buttonAdd.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				// バイナリ検索用のインスタンス生成.
				BinaryPatternInfoCreateDialog binaryDialog = new BinaryPatternInfoCreateDialog(shell, lineFlag);
				// OKボタン押下後処理.
				if (binaryDialog.open() == IDialogConstants.OK_ID) {
					m_infoList.getBinaryPatternInfoList().add(binaryDialog.getBinaryInputData());
					m_infoList.update();
				}
		...
		this.m_buttonModify.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (order >= 0) {
					...
					// ダイアログOKボタン押下後処理.
					if (binaryDialog.open() == IDialogConstants.OK_ID) {
						m_infoList.getBinaryPatternInfoList().remove(order);
						m_infoList.getBinaryPatternInfoList().add(order, binaryDialog.getBinaryInputData());
