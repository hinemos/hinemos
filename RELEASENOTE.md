...
- #7221478 Hinemos AIエージェント
  MCP（Model Context Protocol）サーバに対応した、AIエージェント機能を新たに提供します。

## 電話通知
- #7221306 電話通知
  電話・SMSなどの通信機能をAPIで提供するクラウドサービス「Twilio」と連携し、Hinemosから自動で電話やSMSを発信できる電話通知機能を新たに提供します。
...
Hinemos ver.7.2.0で改善された機能は以下の通りです。
...
### OS
- #7220882 Red Hat Enterprise Linux 10 / Oracle Linux 10対応
  対応したコンポーネントは、マネージャ、Webクライアント、ミッションクリティカル機能です
...
# ■下位互換性
  ...
  - エージェント互換性
    Hinemos ver.7.2のマネージャとHinemos ver.6.xのエージェントは互換性があり、各バージョンのエージェントが提供している機能に制限はございません。
    なお、Hinemos ver.6.xのエージェントは、Hinemos ver.7.0/ver.7.1/ver.7.2の新機能は動作対象外となります。

    Hinemos ver.7.2のマネージャとHinemos ver.7.0/ver.7.1のエージェントは互換性がありエージェントが提供している機能に制限はございません。
    なお、Hinemos ver.7.0/ver.7.1のエージェントは、Hinemos ver.7.2の新機能は動作対象外となります。

# ■パッケージ <a name="packages"/>

以下よりダウンロードできます。

https://github.com/hinemos/hinemos/releases/tag/v7.2.0

なお、Windows版マネージャ、Amazon Linux 2023版マネージャの入手方法についてはHinemosポータルサイト (http://www.hinemos.info/) のご相談・お見積依頼フォームにてお問い合わせください。
