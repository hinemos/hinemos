...
/**
 * ジョブ同時実行制御キューの変更コマンドを実行します。
 *
 * @since 6.2.0
 */
public class ModifyJobQueueAction extends AbstractHandler implements IElementUpdater {
	public static final String ID = ModifyJobQueueAction.class.getName();

	private static Log log = LogFactory.getLog(ModifyJobQueueAction.class);
	...
	private Object execute0(ExecutionEvent event) {
		...
		ViewUtil.executeWithActive(JobQueueEditor.class, view -> {
			...
			JobQueueSettingDialog dialog = new JobQueueSettingDialog(
					HandlerUtil.getActiveWorkbenchWindow(event).getShell(), EditMode.MODIFY, setting,
					...
					(managerName) -> {
						...
						try {
							...
							ModifyJobQueueRequest request = new ModifyJobQueueRequest();
							RestClientBeanUtil.convertBean(setting, request);
							wrapper.modifyJobQueue(target.getQueueId(), request);

							resultDialog.addSuccess(managerName,
									Messages.get("message.jobqueue.modified", setting.getQueueId()));
							shouldClose = true;
						} catch (Throwable t) {
