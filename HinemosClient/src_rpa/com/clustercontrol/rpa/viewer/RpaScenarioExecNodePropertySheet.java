...
import com.clustercontrol.rpa.composite.action.RpaScenarioExecNodeCopyPropertyAction;
import com.clustercontrol.rpa.composite.action.RpaScenarioExecNodeDeletePropertyAction;
 ...
 * プロパティシートクラス<BR>
 */
public class RpaScenarioExecNodePropertySheet extends TreeViewer {
	 ...
	 *            テーブルツリー
	 */
	public RpaScenarioExecNodePropertySheet(Tree tree) {
		super(tree);
		...
		keyColumn.setText(String.format("%s (%s)"
				,Messages.getString("FACILITY_NAME")
				,Messages.getString("FACILITY_ID")
				));
		valueColumn = new TreeColumn(tree, SWT.NONE);
		WidgetTestUtil.setTestId(this, "value", valueColumn);
		valueColumn.setText(String.format("%s (%s)"
				,Messages.getString("RPA_SCENARIO_NAME")
				,Messages.getString("RPA_SCENARIO_ID")
				));
		setSize(200,200);
		...
		//セル・モディファイアを設定
		setCellModifier(new RpaScenarioExecNodePropertySheetModifier(this));
	...
	private void fillContextMenu(IMenuManager manager) {
		//ポップアップメニューにアクションを追加
		manager.add(new RpaScenarioExecNodeCopyPropertyAction(this));
		manager.add(new RpaScenarioExecNodeDeletePropertyAction(this));
	}
