...
@Provider
public class InfraRestFilterRegistration implements DynamicFeature {
	private static final Log log = LogFactory.getLog(InfraRestFilterRegistration.class);

	@Override
	public void configure(ResourceInfo resourceInfo, FeatureContext context) {
		if (log.isDebugEnabled()) {
			log.debug("InfraRestFilterRegistration : resourceMethod=" + resourceInfo.getResourceClass().getName() + "#"
					+ resourceInfo.getResourceMethod().getName());
		}
		if (InfraRestEndpoints.class.isAssignableFrom(resourceInfo.getResourceClass())) {
			//Bearer認証を設定
