...
/**
 * 監視[イベント]ビューの確認中アクションによるイベントの確認の更新処理を行うアクライアント側アクションクラス<BR>
 */
public class EventConfirmingAction extends AbstractHandler implements IElementUpdater {

	// ログ
	private static Log m_log = LogFactory.getLog( EventConfirmingAction.class );

	/** アクションID */
	public static final String ID = EventConfirmingAction.class.getName();
	...
	/**
	 * 監視[イベント]ビューの選択されたアイテムを確認中に更新し、ビューを更新します。
	 * <p>
	 * <ol>
	 * <li>監視[イベント]ビューで、選択されているアイテムを取得します。</li>
	 * <li>取得したイベント情報の確認状態を確認中に更新します。 </li>
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (ClusterControlPlugin.getDefault().getPreferenceStore().getBoolean(MonitorPreferencePage.P_EVENT_CONFIRM_DIALOG_FLG) 
			&& !MessageDialog.openConfirm(
				...
				Messages.getString("dialog.monitor.events.confirming.confirm"))) {
			// OKが押されない場合は処理しない
			return null;
		}
		...
		for(Map.Entry<String, List<List<String>>> entry : map.entrySet()) {
			...
			if (eventInfoList != null && eventInfoList.size()>0) {
				try {
					...
					modifyConfirmRequest.setConfirmType(ConfirmConstant.TYPE_CONFIRMING);
	...
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		...
		if (view.getConfirmTypeList() == null ||
				view.getConfirmTypeList().contains(ConfirmConstant.TYPE_CONFIRMING)) {
				//全く選択されていないか、確認中が選択されている場合、非活性
				editEnable = false;
		}
