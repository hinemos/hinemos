 ...
 *
 */
public class CreateFileCheckJobAction extends AbstractHandler implements IElementUpdater {
	/** ログ */
	private static Log m_log = LogFactory.getLog(CreateFileCheckJobAction.class);
	/** アクションID */
	public static final String ID = CreateFileCheckJobAction.class.getName();
	...
	/**
	 * ジョブ[一覧]ビューの「ファイルチェックジョブの作成」が押された場合に、ファイルチェックジョブを作成します。
	 * <p>
	 * <ol>
	 * <li>ジョブ[一覧]ビューから親となるジョブツリーアイテムを取得します。</li>
	 * <li>ファイルチェックジョブ用のジョブ情報を作成し、親のジョブツリーアイテムの子として追加します。</li>
	 * <li>ジョブ[ファイルチェックジョブの作成・変更]ダイアログを表示します。</li>
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (parent != null) {
			JobInfoWrapper jobInfo = JobTreeItemUtil.getNewJobInfo(parent.getData().getJobunitId(),
					JobInfoWrapper.TypeEnum.FILECHECKJOB);
