	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		for(Map.Entry<String, List<String[]>> map : dataMap.entrySet()) {
			...
			try{
				...
				m_log.warn("run() setStatusCollector monitorId=" + String.join(",", monitorIdList) + ", " + HinemosMessage.replace(e.getMessage()));
				hasRole = false;
			}catch (Exception e) {
				if (failureList.length() > 0) {
					failureList.append("\n");
				...
				failureList.append(HinemosMessage.replace(e.getMessage()) + " : " + String.join(",", monitorIdList));
				m_log.warn("run() setStatusCollector monitorId=" + monitorId + ", " + HinemosMessage.replace(e.getMessage()));
			}
		}
