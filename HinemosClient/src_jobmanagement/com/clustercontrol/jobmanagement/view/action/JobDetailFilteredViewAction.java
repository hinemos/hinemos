...
/*
 * ジョブ履歴[ジョブ詳細]ビューでフィルタ設定によるフィルタ表示の切り替え
 */
public class JobDetailFilteredViewAction extends AbstractHandler implements IElementUpdater {
	/* ログ */
	private static Log log = LogFactory.getLog(JobDetailFilteredViewAction.class);
	
	/* アクションID */
	public static final String ID = JobDetailFilteredViewAction.class.getName();
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (e.widget instanceof ToolItem) {
			view.getComposite().getJobDetailViewModel().setFilteredView(((ToolItem)e.widget).getSelection());
		// ポップアップメニューからの切り替えか？
		} else if (e.widget instanceof MenuItem) {
			view.getComposite().getJobDetailViewModel().setFilteredView(((MenuItem)e.widget).getSelection());
	...
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		...
		if (site.getPart() instanceof JobDetailView) {
			element.setChecked(((JobDetailView)site.getPart()).getComposite().getJobDetailViewModel().isFilteredView());
		}
	}
