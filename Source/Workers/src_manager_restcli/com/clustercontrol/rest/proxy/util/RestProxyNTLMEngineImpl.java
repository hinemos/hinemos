...
// from https://github.com/apache/httpcomponents-client/blob/master/httpclient5/src/main/java/org/apache/hc/client5/http/impl/auth/NTLMEngineImpl.java
 ...
 * @since 4.1
 */
final class RestProxyNTLMEngineImpl implements NTLMEngine {
	...
	static final int FLAG_REQUEST_UNICODE_ENCODING = 0x00000001; // Unicode
																	// string
																	// encoding
																	// requested
	static final int FLAG_REQUEST_OEM_ENCODING = 0x00000002; // OEM string
																// encoding
																// requested
	static final int FLAG_REQUEST_TARGET = 0x00000004; // Requests target field
	static final int FLAG_REQUEST_SIGN = 0x00000010; // Requests all messages
														// have a signature
														// attached, in
														// NEGOTIATE message.
	static final int FLAG_REQUEST_SEAL = 0x00000020; // Request key exchange for
														// message
														// confidentiality in
														// NEGOTIATE message.
														// MUST be used in
														// conjunction with
														// 56BIT.
	static final int FLAG_REQUEST_LAN_MANAGER_KEY = 0x00000080; // Request Lan
																// Manager key
																// instead of
																// user session
																// key
	static final int FLAG_REQUEST_NTLMv1 = 0x00000200; // Request NTLMv1
														// security. MUST be set
														// in NEGOTIATE and
														// CHALLENGE both
	static final int FLAG_DOMAIN_PRESENT = 0x00001000; // Domain is present in
														// message
	static final int FLAG_WORKSTATION_PRESENT = 0x00002000; // Workstation is
															// present in
															// message
	static final int FLAG_REQUEST_ALWAYS_SIGN = 0x00008000; // Requests a
															// signature block
															// on all messages.
															// Overridden by
															// REQUEST_SIGN and
															// REQUEST_SEAL.
	static final int FLAG_REQUEST_NTLM2_SESSION = 0x00080000; // From server in
																// challenge,
																// requesting
																// NTLM2 session
																// security
	static final int FLAG_REQUEST_VERSION = 0x02000000; // Request protocol
														// version
	static final int FLAG_TARGETINFO_PRESENT = 0x00800000; // From server in
															// challenge
															// message,
															// indicating
															// targetinfo is
															// present
	static final int FLAG_REQUEST_128BIT_KEY_EXCH = 0x20000000; // Request
																// explicit
																// 128-bit key
																// exchange
	static final int FLAG_REQUEST_EXPLICIT_KEY_EXCH = 0x40000000; // Request
																	// explicit
																	// key
																	// exchange
	static final int FLAG_REQUEST_56BIT_ENCRYPTION = 0x80000000; // Must be used
																	// in
																	// conjunction
																	// with SEAL
	...
	static final int MSV_AV_EOL = 0x0000; // Indicates that this is the last
											// AV_PAIR in the list.
	static final int MSV_AV_NB_COMPUTER_NAME = 0x0001; // The server's NetBIOS
														// computer name.
	static final int MSV_AV_NB_DOMAIN_NAME = 0x0002; // The server's NetBIOS
														// domain name.
	static final int MSV_AV_DNS_COMPUTER_NAME = 0x0003; // The fully qualified
														// domain name (FQDN) of
														// the computer.
	...
	static final int MSV_AV_FLAGS = 0x0006; // A 32-bit value indicating server
											// or client configuration.
	static final int MSV_AV_TIMESTAMP = 0x0007; // server local time
	static final int MSV_AV_SINGLE_HOST = 0x0008; // A Single_Host_Data
													// structure.
	static final int MSV_AV_TARGET_NAME = 0x0009; // The SPN of the target
													// server.
	static final int MSV_AV_CHANNEL_BINDINGS = 0x000A; // A channel bindings
														// hash.

	static final int MSV_AV_FLAGS_ACCOUNT_AUTH_CONSTAINED = 0x00000001; // Indicates
																		// to
																		// the
																		// client
																		// that
																		...
																		// is
																		// constrained.
	static final int MSV_AV_FLAGS_MIC = 0x00000002; // Indicates that the client
													// is providing message
													// integrity in the MIC
													// field in the
													// AUTHENTICATE_MESSAGE.
	static final int MSV_AV_FLAGS_UNTRUSTED_TARGET_SPN = 0x00000004; // Indicates
																		// that
																		// the
																		// client
																		// is
	...
	private static final byte[] MAGIC_TLS_SERVER_ENDPOINT = "tls-server-end-point:"
	...
	static String getType3Message(final String user, final String password, final String host, final String domain,
			...
			throws NTLMEngineException {
		return new Type3Message(domain, host, user, password, nonce, type2Flags, target, targetInformation,
				peerServerCertificate, type1Message, type2Message).getResponse();
	}
	...
	protected static class CipherGen {
		...
		@Deprecated
		public CipherGen(final String domain, final String user, final String password, final byte[] challenge,
				final String target, final byte[] targetInformation, final byte[] clientChallenge,
			...
			this(RND_GEN, System.currentTimeMillis(), domain, user, password, challenge, target, targetInformation,
					clientChallenge, clientChallenge2, secondaryKey, timestamp);
		}

		public CipherGen(final Random random, final long currentTime, final String domain, final String user,
				final String password, final byte[] challenge, final String target, final byte[] targetInformation,
		...
		@Deprecated
		public CipherGen(final String domain, final String user, final String password, final byte[] challenge,
				final String target, final byte[] targetInformation) {
			this(RND_GEN, System.currentTimeMillis(), domain, user, password, challenge, target, targetInformation);
		}

		public CipherGen(final Random random, final long currentTime, final String domain, final String user,
				final String password, final byte[] challenge, final String target, final byte[] targetInformation) {
			this(random, currentTime, domain, user, password, challenge, target, targetInformation, null, null, null,
					null);
		}
	...
	private static byte[] lmHash(final String password) throws NTLMEngineException {
		try {
			final byte[] oemPassword = password.toUpperCase(Locale.ROOT).getBytes(Charset.forName("US-ASCII"));
	...
	static class NTLMMessage {
		...
		protected int readUShort(final int position) throws NTLMEngineException {
			return RestProxyNTLMEngineImpl.readUShort(messageContents, position);
		}

		/** Read a ulong from a position within the message buffer */
		protected int readULong(final int position) throws NTLMEngineException {
			return RestProxyNTLMEngineImpl.readULong(messageContents, position);
		}

		/** Read a security buffer from a position within the message buffer */
		protected byte[] readSecurityBuffer(final int position) throws NTLMEngineException {
			return RestProxyNTLMEngineImpl.readSecurityBuffer(messageContents, position);
		}
		 ...
		 *
		 * @param maxlength
		 *            the maximum length of the response to prepare, including
	...
	static class Type1Message extends NTLMMessage {
		...
		private int getDefaultFlags() {
			...
			// Protocol version request
					...
					// These must be set according to documentation, based on
					// use of SEAL above
	...
	static class Type2Message extends NTLMMessage {
		...
		Type2Message(final byte[] message) throws NTLMEngineException {
			...
			// Next 8 bytes, starting at offset 12: target field (2 ushort
			// lengths, 1 ulong offset)
			...
			// Next 8 bytes, starting at offset 40: targetinfo field (2 ushort
			// lengths, 1 ulong offset)
	...
	static class Type3Message extends NTLMMessage {
		 ...
		 * More primitive constructor: don't include cert or previous messages.
		 */
		Type3Message(final Random random, final long currentTime, final String domain, final String host,
				final String user, final String password, final byte[] nonce, final int type2Flags, final String target,
				final byte[] targetInformation) throws NTLMEngineException {
			this(random, currentTime, domain, host, user, password, nonce, type2Flags, target, targetInformation, null,
					null, null);
		}
		...
		Type3Message(final String domain, final String host, final String user, final String password,
				...
				final Certificate peerServerCertificate, final byte[] type1Message, final byte[] type2Message)
				throws NTLMEngineException {
			this(RND_GEN, System.currentTimeMillis(), domain, host, user, password, nonce, type2Flags, target,
					targetInformation, peerServerCertificate, type1Message, type2Message);
		}
		...
		Type3Message(final Random random, final long currentTime, final String domain, final String host,
			...
			// Create a cipher generator class. Use domain BEFORE it gets
			// modified!
			final CipherGen gen = new CipherGen(random, currentTime, unqualifiedDomain, user, password, nonce, target,
			...
			byte[] userSessionKey;
			try {
				// This conditional may not work on Windows Server 2008 R2 and
				// above, where it has not yet
				// been tested
		...
		protected void buildMessage() {
			...
			addULong(
					 ...
					 * 
					 * // Required flags (type2Flags &
					 * FLAG_REQUEST_LAN_MANAGER_KEY) | (type2Flags &
					 * FLAG_REQUEST_NTLMv1) | (type2Flags &
					 * FLAG_REQUEST_NTLM2_SESSION) |
					 * 
					 * // Protocol version request FLAG_REQUEST_VERSION |
					 * 
					 * // Recommended privacy settings (type2Flags &
					 * FLAG_REQUEST_ALWAYS_SIGN) | (type2Flags &
					 * FLAG_REQUEST_SEAL) | (type2Flags & FLAG_REQUEST_SIGN) |
					 * 
					 * // These must be set according to documentation, based on
					 * use of SEAL above (type2Flags &
					 * FLAG_REQUEST_128BIT_KEY_EXCH) | (type2Flags &
					 * FLAG_REQUEST_56BIT_ENCRYPTION) | (type2Flags &
					 * FLAG_REQUEST_EXPLICIT_KEY_EXCH) |
					 * 
					 * (type2Flags & FLAG_TARGETINFO_PRESENT) | (type2Flags &
					 * FLAG_REQUEST_UNICODE_ENCODING) | (type2Flags &
					 * FLAG_REQUEST_TARGET)
					 */
					type2Flags);
			...
			if (computeMic) {
				// Computation of message integrity code (MIC) as specified in
				// [MS-NLMP] section 3.2.5.1.2.
		...
		/**
		 * Add GSS channel binding hash and MIC flag to the targetInfo. Looks
		 * like this is needed if we want to use exported session key for GSS
		 * wrapping.
		 */
	...
	@Override
	public String generateType3Msg(final String username, final char[] password, final String domain,
			final String workstation, final String challenge) throws NTLMEngineException {
		return generateType3Msg(username, String.valueOf(password), domain, workstation, challenge);
	}
