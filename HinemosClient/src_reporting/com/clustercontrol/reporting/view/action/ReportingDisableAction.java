...
/**
 * レポーティング[スケジュール]ビューの[無効]アクションクラス<BR>
 * 
 * @version 5.0.a
 * @since 4.1.2
 */

public class ReportingDisableAction extends AbstractHandler implements IElementUpdater {

	// ログ
	private static Log m_log = LogFactory.getLog(ReportingDisableAction.class);

	/** アクションID */
	public static final String ID = ReportingDisableAction.class.getName();
...
//	public void run(IAction action) {
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!MessageDialog.openConfirm(null, 
				Messages.getString("confirmed"),
				Messages.getString("message.reporting.15", args))) {
			return null;
		}
		...
		for(Map.Entry<String, List<String>> entry : map.entrySet()) {
			...
			for(String scheduleId : entry.getValue()) {
				try {
					wrapper.setReportingStatus(scheduleId, false);
		...
		if (successList.length() != 0) {
			...
			MessageDialog.openInformation(null,
					...
					Messages.getString("message.reporting.20", args));
		}
		...
		if (failureList.length() != 0) {
			...
			MessageDialog.openError(null, 
					...
					Messages.getString("message.reporting.21", args));
		}
