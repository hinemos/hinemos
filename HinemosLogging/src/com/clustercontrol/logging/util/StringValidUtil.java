...
public class StringValidUtil {
	...
	public static void setReplaceChar(String replaceChar) {
		StringValidUtil.replaceChar = replaceChar;
	}
	...
	public static void setReplace(boolean replace) {
		StringValidUtil.replace = replace;
	}

	/**
	 * 入力文字列datavalueについて扱えない文字を削除した文字列を返却する
	 * 
	 * @param datavalue
	 * @return
	 */
	...
	/**
	 * dataarrayの中に不正な文字列が含まれるか否か
	 * 
	 * @param dataarray
	 * @return
	 */
	public static boolean existInvalidCharactor(char[] dataarray) {
		...
		while (i < datalength) {
			...
			if (StringValidUtil.isInvalid(ch0)) {

				// check if this is a supplemental character
				if (StringValidUtil.isHighSurrogate(ch0) && i < datalength) {

					char ch1 = dataarray[i++];
					if (StringValidUtil.isLowSurrogate(ch1)
							&& StringValidUtil.isSupplemental(StringValidUtil.supplemental(ch0, ch1))) {
						continue;
					}
				}
