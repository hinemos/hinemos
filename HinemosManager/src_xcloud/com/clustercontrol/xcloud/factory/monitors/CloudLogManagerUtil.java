...
/**
 * クラウドログ監視の設定変更をエージェントに通知するクラス
 *
 */
public class CloudLogManagerUtil{

	private static Log m_log = LogFactory.getLog(CloudLogManagerUtil.class);

	/**
	 * topicを送信します。
	 * topicを受信したエージェントはクラウドログ監視の設定を再取得します。
	...
	public static void broadcastConfigured() {
		...
		// MAIN
		m_log.info("broadcasting cloudlog monitor configuration modified.");

		TopicInfo topicInfo = new TopicInfo();
		topicInfo.setCloudLogMonitorChanged(true);
	...
	public static void broadcastConfiguredFlowControl() {
		...
		m_log.info("broadcasting cloudlog monitor configuration modified. with flow control.");
	...
	/**
	 * 特定エージェントに対して、クラウドログ監視設定変更を通知する
	 * 
	...
	public static void specificcastConfigured(String facilityId) {
		...
		// MAIN
		m_log.info("specificcasting cloudlog monitor configuration modified.");
