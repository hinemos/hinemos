...
public class MonitorSettingControllerBean {
	...
	public MonitorInfo addMonitor(MonitorInfo info, boolean isImport)
		...
		try {
			...
			// HTTP 監視
				...
				// それ以外
					...
					HinemosUnknown e = new HinemosUnknown(
							"This monitorTypeId = " + monitorTypeId + ", but unknown monitorType = " + monitorType);
			...
			// Other(Pluginで追加する汎用的な監視)
				...
				if (addMonitor == null) {
					HinemosUnknown e = new HinemosUnknown(
							"Unknown monitorTypeId = " + monitorTypeId + ", monitorType = " + monitorType);
					m_log.info("addMonitor() : "
							+ e.getClass().getSimpleName() + ", " + e.getMessage());
					throw e;
				}
			...
			try {
				...
				if (e.getCause() instanceof PersistenceException) {
					...
					throw new HinemosUnknown(
							"MonitorSettingControllerBean.modifyMonitor, monitorId = " + info.getMonitorId(),
							e.getCause());
				} else {
					throw e;
				}
				...
				jtm.rollback();
				throw new HinemosUnknown(
						"MonitorSettingControllerBean.modifyMonitor, monitorId = " + info.getMonitorId(), e);
			} catch (Exception e) {
				m_log.warn("modifyMonitor() : "
						+ e.getClass().getSimpleName() + ", " + e.getMessage(), e);
				jtm.rollback();
		...
		try {
			...
			for (MonitorInfo monitorInfo : list) {
				...
				if (monitorInfo.getMonitorType().equals(MonitorTypeConstant.TYPE_NUMERIC)) {
					...
					monitorInfo.setPredictionNotifyRelationList(
							NotifyRelationCache.getNotifyList(
									CollectMonitorManagerUtil
											.getPredictionNotifyGroupId(monitorInfo.getNotifyGroupId())));
					// 通知情報(変化点監視)の取得
					monitorInfo.setChangeNotifyRelationList(
							NotifyRelationCache.getNotifyList(
									CollectMonitorManagerUtil.getChangeNotifyGroupId(monitorInfo.getNotifyGroupId())));
	...
	public List<MonitorInfo> setStatusMonitor(List<String> monitorIdList, boolean validFlag)
			throws HinemosUnknown, MonitorNotFound, InvalidRole {
		List<MonitorInfo> retList = new ArrayList<>();
		...
		String user = HinemosSessionContext.instance().getProperty(HinemosSessionContext.PROPERTY_KEY_USER_ID);
		...
		try {
			...
			for (String monitorId : monitorIdList) {
				...
				MonitorInfo info = jtm.getEntityManager().find(MonitorInfo.class, monitorId,
						ObjectPrivilegeMode.MODIFY);
				if (info == null) {
					throw new MonitorNotFound("MonitorInfo.findByPrimaryKey, monitorId = " + monitorId);
				}
				...
				if (isChangeNecessary(info, validFlag)) {
					...
					new ModifySchedule().updateSchedule(info, jtm);
					new NotifyControllerBean().deleteNotifyHistory(info.getMonitorTypeId(), info.getMonitorId(), jtm);
				}
				retList.add(info);
			}
			jtm.commit();
	 ...
	 * @throws MonitorNotFound
	 */
	public List<MonitorInfo> setStatusCollector(List<String> monitorIdList, boolean validFlag)
			throws HinemosUnknown, MonitorNotFound, InvalidRole {
		List<MonitorInfo> retList = new ArrayList<>();
			...
			for (String monitorId : monitorIdList) {
				...
				if (validFlag) {
					...
					if (!info.getCollectorFlg()) {
						...
						new ModifySchedule().updateSchedule(info, jtm);
						new NotifyControllerBean().deleteNotifyHistory(info.getMonitorTypeId(), info.getMonitorId(),
								jtm);
					}
				} else {
					if (info.getCollectorFlg()) {
	 ...
	 * @throws HinemosUnknown
	 */
	public ArrayList<MonitorInfo> getMonitorListWithoutCheckInfo(MonitorFilterInfo condition)
			throws InvalidRole, HinemosUnknown {
		...
		try {
			...
			m_log.info(
					"getMonitorListWithoutCheckInfo(conditionList) " + e.getClass().getName() + ", " + e.getMessage());
			jtm.rollback();
			throw new HinemosUnknown(e.getMessage(), e);
		} catch (ObjectPrivilege_InvalidRole e) {
			if (jtm != null)
			...
			String msg = String.format(
					"getMonitorListWithoutCheckInfo() end : term1=%s[msec], term2=%s[msec], all_term=%s[msec], ",
	 ...
	 * @throws HinemosUnknown
	 */
	public ArrayList<MonitorInfoBean> getMonitorBeanListWithoutCheckInfo(MonitorFilterInfo condition)
			throws InvalidRole, HinemosUnknown {
		m_log.debug("getMonitorListWithoutCheckInfo(conditionList) : start");
	...
	public List<String> getMonitorStringTagList(String monitorId, String ownerRoleId)
		...
		try {
			...
			if (SdmlUtil.isCreatedBySdml(monitorInfo)) {
				...
				retSet.addAll(
						SdmlUtil.getSampleTagList(monitorInfo.getMonitorId(), monitorInfo.getSdmlMonitorTypeId()));
			}
	...
	public ArrayList<MonitorInfo> getMonitorListForLogcount(String facilityId, String ownerRoleId)
		...
		try {
			...
			List<MonitorInfo> monitorInfoList = new SelectMonitor().getMonitorListByMonitorType_OR(typeList,
					ownerRoleId);
