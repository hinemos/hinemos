...
/**
 * RPA設定[シナリオ実績作成設定]ビューの無効アクションクラス<BR>
 */
public class RpaScenarioCreateSettingDisableAction extends AbstractHandler implements IElementUpdater {

	/** ログ */
	private static Log log = LogFactory.getLog(RpaScenarioCreateSettingDisableAction.class);

	/** アクションID */
	public static final String ID = RpaScenarioCreateSettingDisableAction.class.getName();
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		// 選択アイテムの取得
				...
				if (!MessageDialog.openConfirm(
						...
						Messages.getString("message.rpa.scenario.create.setting.disable.confirm", new String[]{String.join("\n", idList)}))) {

					// OKが押されない場合は終了
					return null;
				}

				Map<String, List<String>> disableMap = null;

				for(String[] args : argsList) {
					String managerName = args[0];
					String scenarioId = args[1];
					if(disableMap == null) {
						disableMap = new ConcurrentHashMap<String, List<String>>();
					}
					if(disableMap.get(managerName) == null) {
						disableMap.put(managerName, new ArrayList<String>());
					}
					disableMap.get(managerName).add(scenarioId);
				}
				...
				if (!(successManager.length()==0)) {
					MessageDialog.openInformation(null, Messages.getString("successful"), 
							Messages.getString("message.rpa.scenario.create.setting.disable.finish", new String[]{successManager.toString()}));
					view.update();
				}
		return null;
	}
