...
import com.clustercontrol.rest.endpoint.utility.dto.ImportEventFilterSettingRecordRequest;
...
public class ImportFilterSettingMonitorHistoryEventController extends AbstractImportController<ImportEventFilterSettingRecordRequest,RecordRegistrationResponse>{

	public ImportFilterSettingMonitorHistoryEventController(boolean isRollbackIfAbnormal, List<ImportEventFilterSettingRecordRequest> importList) {
		super(isRollbackIfAbnormal, importList);
	}
	
	public RecordRegistrationResponse proccssRecord( ImportEventFilterSettingRecordRequest importRec ) throws Exception {
		...
		if(importRec.getIsNewRecord()){
			AddEventFilterSettingRequest requestDto = new AddEventFilterSettingRequest();
			...
			ModifyEventFilterSettingRequest requestDto = new ModifyEventFilterSettingRequest();
			RestBeanUtil.convertBeanSimple(importRec.getImportData(), requestDto);
	...
	private void setCommonProperty(ImportEventFilterSettingRecordRequest importRec, 
			FilterSettingInfo infoReq){
		// 共通フィルタorユーザフィルタで分岐
		if(importRec.getImportData().getCommon()){
			infoReq.setFilterCategory(FilterCategoryEnum.EVENT);
