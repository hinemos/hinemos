...
/*
 * ジョブ履歴[ジョブ詳細]ビューでフラット表示の切り替え
 */
public class JobDetailFlatViewAction extends AbstractHandler implements IElementUpdater {
	/* ログ */
	private static Log log = LogFactory.getLog(JobDetailFlatViewAction.class);
	
	/* アクションID */
	public static final String ID = JobDetailFlatViewAction.class.getName();
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (e.widget instanceof ToolItem) {
			view.getComposite().getJobDetailViewModel().setFlatView(((ToolItem)e.widget).getSelection());
		// ポップアップメニューからの切り替えか？
		} else if (e.widget instanceof MenuItem) {
			view.getComposite().getJobDetailViewModel().setFlatView(((MenuItem)e.widget).getSelection());
	...
	public void updateElement(UIElement element, @SuppressWarnings("rawtypes") Map parameters) {
		...
		if (site.getPart() instanceof JobDetailView) {
			element.setChecked(((JobDetailView)site.getPart()).getComposite().getJobDetailViewModel().isFlatView());
		}
	}
