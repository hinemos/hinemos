...
public class SuspendInstanceHandler extends AbstractCloudOptionHandler implements CloudStringConstants {
	@Override
	public Object internalExecute(ExecutionEvent event) throws CloudManagerException, InvalidUserPass, InvalidRole, RestConnectFailed, HinemosUnknown {
		...
		if (MessageDialog.openConfirm(
				...
				instanceIds.size() > 1 ? MessageFormat.format(msgConfirmSuspendComputeNodeMulti, instanceIds.size()):
					MessageFormat.format(msgConfirmSuspendComputeNode, instance.getName(), instance.getId()))) {
			...
			endpoint.suspendInstances(instance.getCloudScope().getId(), instance.getLocation().getId(), String.join(",", instanceIds));

			// 成功報告ダイアログを生成
			MessageDialog.openInformation(
					null,
					Messages.getString("successful"),
					msgFinishSuspendComputeNode);
	...
	protected String getErrorMessage() {
		return msgErrorFinishSuspendComputeNode;
	}
}
