			...
			if (!first) {
				...
				if (jobStartJobMstEntityList != null) {
					for(JobStartJobMstEntity jobStartJobMstEntity : jobStartJobMstEntityList) {
						...
						jobStartJobInfoEntity.setTargetJobCrossSessionRange(jobStartJobMstEntity.getTargetJobCrossSessionRange());
						...
						m_log.debug("getTargetJobCrossSessionRange = " + jobStartJobMstEntity.getTargetJobCrossSessionRange());
						// 重複チェック
						jtm.checkEntityExists(JobStartJobInfoEntity.class, jobStartJobInfoEntity.getId());
						// 登録
						jtm.getEntityManager().persist(jobStartJobInfoEntity);
						jobStartJobInfoEntity.relateToJobInfoEntity(jobInfoEntity);
						//ジョブ変数待ち条件、セッション横断ジョブ待ち条件の場合フラグをtrueにする
						if (jobStartJobInfoEntity.getId().getTargetJobType() == JudgmentObjectConstant.TYPE_CROSS_SESSION_JOB_END_STATUS 
							|| jobStartJobInfoEntity.getId().getTargetJobType() == JudgmentObjectConstant.TYPE_CROSS_SESSION_JOB_END_VALUE) {
							waitCheckFlg = true;
						}
					}
				}
				...
				if (jobStartParamMstEntityList != null) {
					for (JobStartParamMstEntity jobStartParamMstEntity : jobStartParamMstEntityList) {
						...
						em.persist(jobStartParamInfoEntity);
						jobStartParamInfoEntity.relateToJobInfoEntity(jobInfoEntity);
