...
/**
 *  SummaryDayにマッピングするテーブルからデータを削除するクラス
 */
public class SummaryDayJdbcBatchDelete extends JdbcBatchQuery {
	private static final String SQL = "DELETE FROM log.cc_collect_summary_day "
			+ "WHERE collector_id = ? "
			+ "AND time >= ? "
			+ "AND time <= ? ";
	...
	public SummaryDayJdbcBatchDelete(List<CollectKeyInfo> entities, Long fromTime, Long toTime) {
