...
/**
 * {@link JobQueueActivityView}をフィルタリングするコマンドを実行します。
 *
 * @version 6.2.0
 */
public class FilterJobQueueActivityAction extends AbstractHandler {
	public static final String ID = FilterJobQueueActivityAction.class.getName();

	private static Log log = LogFactory.getLog(FilterJobQueueActivityAction.class);
	...
	private Object execute0(ExecutionEvent event) throws Exception {
		...
		JobQueueActivityView view = ViewUtil.findActive(JobQueueActivityView.class);
		...
		if (beforeValue) {
			...
			if (dialog.open() == IDialogConstants.OK_ID) {
				view.enableFilter(dialog.getManagerFilter(), dialog.getQueueFilterForActivity());
				view.update();
			} else {
