 ...
 * @author modified by NTT DATA INTELLILINK Corporation
 */
...
public class ServerRuntime {
	...
	static ServerRuntime createServerRuntime(InjectionManager injectionManager, ServerBootstrapBag bootstrapBag,
			Stage<RequestProcessingContext> processingRoot, ApplicationEventListener eventListener,
			ProcessingProviders processingProviders) {

		ScheduledExecutorService scheduledExecutorServiceSupplier = injectionManager
				.getInstance(ScheduledExecutorService.class, BackgroundSchedulerLiteral.INSTANCE);

		Provider<ExecutorService> asyncExecutorServiceSupplier = () -> injectionManager
				.getInstance(ExecutorService.class, ManagedAsyncExecutorLiteral.INSTANCE);

		return new ServerRuntime(processingRoot, processingProviders, injectionManager,
				scheduledExecutorServiceSupplier, asyncExecutorServiceSupplier, bootstrapBag.getRequestScope(),
				bootstrapBag.getExceptionMappers(), eventListener,
				injectionManager.getInstance(ExternalRequestScope.class), bootstrapBag.getConfiguration());
	}

	private ServerRuntime(final Stage<RequestProcessingContext> requestProcessingRoot,
			final ProcessingProviders processingProviders, final InjectionManager injectionManager,
			final ScheduledExecutorService backgroundScheduler,
			final Provider<ExecutorService> managedAsyncExecutorProvider, final RequestScope requestScope,
			final ExceptionMappers exceptionMappers, final ApplicationEventListener applicationEventListener,
			final ExternalRequestScope externalScope, final Configuration configuration) {
		...
		this.processResponseErrors = PropertiesHelper
				.isProperty(configuration.getProperty(ServerProperties.PROCESSING_RESPONSE_ERRORS_ENABLED));

		this.disableLocationHeaderRelativeUriResolution = ServerProperties.getValue(configuration.getProperties(),
				ServerProperties.LOCATION_HEADER_RELATIVE_URI_RESOLUTION_DISABLED, Boolean.FALSE, Boolean.class);

		this.rfc7231LocationHeaderRelativeUriResolution = ServerProperties.getValue(configuration.getProperties(),
				ServerProperties.LOCATION_HEADER_RELATIVE_URI_RESOLUTION_RFC7231, Boolean.FALSE, Boolean.class);
	}
	...
	public void process(final ContainerRequest request) {
		...
		if (applicationEventListener != null) {
			...
			monitoringEventListener = applicationEventListener
					.onRequest(monitoringEventBuilder.build(RequestEvent.Type.START));
		}

		request.setProcessingProviders(processingProviders);

		final RequestProcessingContext context = new RequestProcessingContext(injectionManager, request, routingContext,
				monitoringEventBuilder, monitoringEventListener);
		...
		final AsyncResponderHolder asyncResponderHolder = new AsyncResponderHolder(responder, externalRequestScope,
			...
			public void run() {
				try {
					...
					if (!disableLocationHeaderRelativeUriResolution) {
						final URI uriToUse = rfc7231LocationHeaderRelativeUriResolution ? request.getRequestUri()
								: request.getBaseUri();
	...
	/**
	 * Ensure that the value a {@value HttpHeaders#LOCATION} header is an
	 * absolute URI, if present among headers.
	 * <p/>
	 * Relative URI value will be made absolute using a base request URI.
	 *
	 * @param location
	 *            location URI; value of the HTTP {@value HttpHeaders#LOCATION}
	 *            response header.
	 * @param headers
	 ...
	 *            container request.
	 * @param incompatible
	 *            if set to {@code true}, uri will be resolved against the
	 *            request uri, not the base uri; this is correct against
	 *            RFC7231, but does violate the JAX-RS 2.0 specs
	 */
	private static void ensureAbsolute(final URI location, final MultivaluedMap<String, Object> headers,
		...
		// according to RFC7231 (HTTP/1.1), this field can contain one single
		// URI reference
	...
	private static class AsyncResponderHolder implements Value<AsyncContext> {
		...
		private AsyncResponderHolder(final Responder responder, final ExternalRequestScope externalRequestScope,
				final RequestContext requestContext, final ExternalRequestContext<?> externalContext) {
	...
	private static class Responder {
		...
		/**
		 * Process {@code throwable} by using exception mappers and generating
		 * the mapped response if possible.
		 * <p>
		 * Note about logging:
		 * <ul>
		 * <li>we do not log exceptions that are mapped by ExceptionMappers.
		 * </li>
		 * <li>All other exceptions are logged: WebApplicationExceptions with
		 * entities, exceptions that were unsuccessfully mapped</li>
		 * </ul>
		 * </p>
		 *
		 * @param throwable
			...
			try {
				...
				try {
					...
					LOGGER.log(Level.SEVERE,
							LocalizationMessages.ERROR_PROCESSING_RESPONSE_FROM_ALREADY_MAPPED_EXCEPTION());
					processingContext.monitoringEventBuilder().setException(respError,
							RequestEvent.ExceptionCause.MAPPED_RESPONSE);
					processingContext.triggerEvent(RequestEvent.Type.ON_EXCEPTION);
					throw respError;
				}
				...
				if (throwable != responseError
					...
					LOGGER.log(Level.FINE, LocalizationMessages.ERROR_EXCEPTION_MAPPING_ORIGINAL_EXCEPTION(),
							throwable);
				}

				if (!processResponseError(responseError)) {
					// Pass the exception to the container.
					LOGGER.log(Level.FINE, LocalizationMessages.ERROR_EXCEPTION_MAPPING_THROWN_TO_CONTAINER(),
							responseError);
		...
		/**
		 * If
		 * {@value org.glassfish.jersey.server.ServerProperties#PROCESSING_RESPONSE_ERRORS_ENABLED}
		 * is set to true then try to handle errors raised during response
		 * processing.
		 *
		 * @param responseError
		 *            a throwable that occurred during response processing.
		 * @return {@code true} if the given response error has been processed,
		...
		private boolean processResponseError(final Throwable responseError) {
			...
			if (runtime.processResponseErrors) {
				...
				try {
					...
					LOGGER.log(Level.FINE, LocalizationMessages.ERROR_EXCEPTION_MAPPING_PROCESSED_RESPONSE_ERROR(),
							throwable);
				}
			}
		...
		private ContainerResponse convertResponse(final Response exceptionResponse) {
			final ContainerResponse containerResponse = new ContainerResponse(processingContext.request(),
					exceptionResponse);
		...
		private Response mapException(final Throwable originalThrowable) throws Throwable {
			...
			do {
				if (throwable instanceof MappableException) {
					...
					// in case
					// ServerProperties.PROCESSING_RESPONSE_ERRORS_ENABLED is
					// true, allow
					...
					if (mapper != null) {
						...
						try {
							...
							if (tracingLogger.isLogEnabled(ServerTraceEvent.EXCEPTION_MAPPING)) {
								tracingLogger.logDuration(ServerTraceEvent.EXCEPTION_MAPPING, timestamp, mapper,
							...
							if (mappedResponse != null) {
								...
								if (LOGGER.isLoggable(Level.FINER)) {
									final String message = String.format(
											...
											throwable.getLocalizedMessage(), mapper.getClass().getName(),
							...
							// spec: If the exception mapping provider throws an
							// exception while creating a Response
							// then return a server error (status code 500)
							// response to the client.
							LOGGER.log(Level.SEVERE,
									LocalizationMessages.EXCEPTION_MAPPER_THROWS_EXCEPTION(mapper.getClass()),
									mapperThrowable);
							LOGGER.log(Level.SEVERE, LocalizationMessages.EXCEPTION_MAPPER_FAILED_FOR_EXCEPTION(),
									throwable);
				...
				if (!inMappable || mappingNotFound) {
					// user failures (thrown from Resource methods or provider
					// methods)

					// spec: Unchecked exceptions and errors that have not been
					// mapped MUST be re-thrown and allowed to
			...
			// jersey failures (not thrown from Resource methods or provider
			// methods) -> rethrow
			...
			try {
				...
				try {
					response.setEntityStream(request.getWorkers().writeTo(entity, entity.getClass(),
							response.getEntityType(), response.getEntityAnnotations(), response.getMediaType(),
							response.getHeaders(), request.getPropertiesDelegate(), response.getEntityStream(),
				...
				if (response.isCommitted()) {
					/**
					 * We're done with processing here. There's nothing we can
					 * do about the exception so let's just log it.
					 */
				...
				if (!skipFinally) {
					...
					if (response.isChunked()) {
						...
						try {
							chunked.setContext(runtime.requestScope, runtime.requestScope.referenceCurrent(), request,
									response, connectionCallbackRunner);
						} catch (final IOException ex) {
							LOGGER.log(Level.SEVERE, LocalizationMessages.ERROR_WRITING_RESPONSE_ENTITY_CHUNK(), ex);
							close = true;
						}
						// suspend the writer indefinitely (passing null timeout
						// handler is ok in such case) if the output is not
						// already closed.
						// TODO what to do if we detect that the writer has
						// already been suspended? override the timeout value?
						if (!chunked.isClosed() && !writer.suspend(AsyncResponder.NO_TIMEOUT, TimeUnit.SECONDS, null)) {
					...
					if (close) {
						try {
							// the response must be closed here instead of just
							// flushed or committed. Some
							// output streams writes out bytes only on close
							// (for example GZipOutputStream).
							response.close();
						} catch (final Exception e) {
		...
		private void release(final ContainerResponse responseContext) {
			try {
				...
				// responseContext may be null in case the request processing
				// was cancelled.
				...
				LOGGER.log(Level.WARNING, LocalizationMessages.RELEASING_REQUEST_PROCESSING_RESOURCES_FAILED(),
						throwable);
			} finally {
	...
	private static class AsyncResponder
			implements AsyncContext, ContainerResponseWriter.TimeoutHandler, CompletionCallback {
		...
		private final Responder responder;
		// TODO this instance should be released once async invocation is
		// finished.
		private final RequestContext requestContext;
		...
		public AsyncResponder(final Responder responder, final RequestContext requestContext,
				final ExternalRequestScope requestScopeListener, final ExternalRequestContext<?> foreignScopeInstance) {
			this.responder = responder;
			this.requestContext = requestContext;
			this.foreignScopeInstance = foreignScopeInstance;
			this.requestScopeListener = requestScopeListener;

			this.callbackRunners = Collections.unmodifiableList(
					Arrays.asList(responder.completionCallbackRunner, responder.connectionCallbackRunner));
		...
		public boolean suspend() {
			synchronized (stateLock) {
				if (state == RUNNING) {
					if (responder.processingContext.request().getResponseWriter().suspend(AsyncResponse.NO_TIMEOUT,
		...
		public boolean resume(final Object response) {
			return resume(new Runnable() {
				...
				public void run() {
					try {
						requestScopeListener.resume(foreignScopeInstance, responder.runtime.injectionManager);
						final Response jaxrsResponse = (response instanceof Response) ? (Response) response
			...
			return resume(new Runnable() {
				...
				public void run() {
					try {
						...
						// Ignore the exception - already resumed but may be
						// rethrown by ContainerResponseWriter#failure.
					}
				}
			});
		}
	 ...
	 * <p/>
	 * The runner supports registering multiple callbacks of a specific type and
	 * the execute the callback method on all the registered callbacks.
	 *
	 * @param <T>
	 *            callback type
	 */
	abstract static class AbstractCallbackRunner<T> {
		 ...
		 *
		 * @param logger
		 *            logger instance to be used by the runner to fire logging
		 *            events.
		 */
		 ...
		 * @return True if this callback runner supports the
		 *         {@code callbackClass}; false otherwise.
		 */
		 ...
		 * @return {@code true} upon successful registration, {@code false}
		 *         otherwise.
		 */
		@SuppressWarnings("unchecked")
		 ...
		 *
		 * @param invoker
		 *            invoker responsible for to executing all registered
		 *            callbacks.
		 */
			...
			for (final T callback : callbacks) {
				try {
					...
					logger.log(Level.WARNING,
							LocalizationMessages.ERROR_ASYNC_CALLBACK_FAILED(callback.getClass().getName()), t);
				}
			}
		}
	}

	private static class CompletionCallbackRunner extends AbstractCallbackRunner<CompletionCallback>
	...
	private static class ConnectionCallbackRunner extends AbstractCallbackRunner<ConnectionCallback>
