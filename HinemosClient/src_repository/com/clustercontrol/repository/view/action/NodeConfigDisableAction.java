...
/**
 * リポジトリ[構成情報収集]ビューの収集無効アクションクラス<BR>
 *
 * @version 6.2.0
 * @since 6.2.0
 */
public class NodeConfigDisableAction extends AbstractHandler implements IElementUpdater {

	// ログ
	private static Log m_log = LogFactory.getLog(NodeConfigDisableAction.class);

	/** アクションID */
	public static final String ID = NodeConfigDisableAction.class.getName();
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!MessageDialog.openConfirm(null, Messages.getString("confirmed"),
				Messages.getString("message.node.config.6", args))) {
			return null;
		}
		...
		for (Map.Entry<String, List<String>> map : dataMap.entrySet()) {
			...
			for (String settingId : map.getValue()) {
				...
				try {
					wrapper.setStatusNodeConfigSetting(settingId, false);
					...
					m_log.warn("run() setStatusNodeConfigSetting settingId=" + settingId + ", " + e.getMessage(), e);
					hasRole = false;
				} catch (Exception e) {
					if (failureList.length() > 0) {
						failureList.append(", ");
		...
		if (successList.length() != 0) {
			...
			MessageDialog.openInformation(null, Messages.getString("successful"),
					Messages.getString("message.node.config.7", args));
		}
		...
		if (failureList.length() != 0) {
			...
			MessageDialog.openError(null, Messages.getString("failed"),
					Messages.getString("message.node.config.8", args));
		}
